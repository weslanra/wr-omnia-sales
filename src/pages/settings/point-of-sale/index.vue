...
const { data: pointsOfSaleData, execute: fetchPointsOfSale } =
  await useApi<any>(
    createUrl("/points-of-sale", {
      query: {
        q: searchQuery,
        page,
        itemsPerPage,
...
const pointsOfSale = computed((): PointOfSale[] => pointsOfSaleData.value.data);
const totalPointsOfSale = computed(() => pointsOfSaleData.value.total);

// 👉 Delete point of sale
const deletePointOfSale = async (id: number) => {
  await $api(`/points-of-sale/${id}`, { method: "DELETE" });
  ...
  <section v-if="pointsOfSale">
    <VCard>
      ...
      <VDataTableServer
        ...
        v-model:page="page"
        show-select
        :items-length="totalPointsOfSale"
        :headers="headers"
        :items="pointsOfSale"
        item-value="id"
        class="text-no-wrap"
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <IconBtn @click="deletePointOfSale(item.id)">
